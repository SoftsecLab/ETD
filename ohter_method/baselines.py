...
def experiment(args):
    ...
    for name in criterion_fns:
        ...
        for idx in tqdm.tqdm(range(n_samples), desc=f"Computing {name} criterion"):
            ...
            with torch.no_grad():
                logits = scoring_model(**tokenized).logits[:, :-1]
                rewritten_crit = criterion_fn(logits, labels)
            # result
...
parser.add_argument('--output_file', type=str, default="./results/",
                    help="Save path prefix for all output experimental results")
parser.add_argument('--dataset', type=str, default="data",
                    help="Short identifier tag of the target dataset")
parser.add_argument('--dataset_file', type=str, default="./data",
                    help="Directory path storing the raw dataset json file")
parser.add_argument('--scoring_model_name', type=str, default="gpt",
                    help="Local folder path or official name of scoring large language model")
parser.add_argument('--seed', type=int, default=0,
                    help="Fixed random seed to ensure experimental reproducibility")
parser.add_argument('--device', type=str, default="cuda",
                    help="Hardware device for model inference, options: cuda, cpu")
parser.add_argument('--cache_dir', type=str, default="./local_model",
                    help="Storage directory for cached downloaded pretrained model weights")
